...
def test_otp_encryption_decryption(
    ...
    sample_seed: bytes = b"SecretSeed123",
) -> bool:
...
def test_otp_offset_stream(
    ...
    c_part1 = encrypt(sample_seed, sample_text[:5], offset=0)
    c_part2 = encrypt(sample_seed, sample_text[5:], offset=5)
...
def run_otp_algebraic_tests() -> bool:
    print("\n" + "=" * 70)
    print("[UNIT TEST] Testing OTP Algebraic Properties (PRNG Seed-based)...")
    print("=" * 70)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="OTP IPC Test Runner (Task 2)"
    )
    parser.add_argument(
        "--generate-key",
        action="store_true",
